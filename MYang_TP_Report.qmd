...
The Fine Root Ecology Database (FRED) contains a collection of research articles that have root trait information. Root traits are variables such as root biomass and specific root length. Documenting these variables can be useful for future research that aims to examine relationships between root traits or help understand belowground ecology. Aboveground ecology has been extensively studied, but root ecology have has not (because it is very difficult and tedious to parse out roots) but in recent years, root ecology has started to gain more recognition as we start to understand the potential impacts and role in ecosystems and climate change (First-Person Science: Colleen Iversen on Belowground Ecology, 2020).
...
One of the data frames we would like to create is a pivot table with columns: article name, latitude, longitude, root trait name, and root trait value. This will give us information about the type of root traits each article records and the root trait value. This will also be the basis for other data frames we will create.
...
The next data frame we will create is one with just the article name and the number of root traits each records. This will simplify a lot of the information, but we will also lose out on the names and values of the root traits. We will use this data frame to join with the data frame that contained site information (latitude and longitude) so that we can plot a leaflet that shows the number of root traits recorded in each site. 
...
The code below combines the data frames as previously mentioned. We only include articles where there are specified coordinates and exclude those where the coordinates are unknown (in the dataset, this is denoted as "unspecified").
...
::: {.callout-important}
The output for the leaflet does not render well on the PDF. Use the HTML file to see the plot.
...
::: {.callout-important}
The output for the plotly figure does not render well on the PDF. Use the HTML file to see the plot.
:::
...
The leaflets and graphs offer a snapshot of the FRED 3.0 and FRED 4.0 databases. From the leaflet, we can explore the specific sites where articles collected root trait information and see how the locations differed between articles in FRED 3.0 and FRED 4.0. 
...
For further exploration, we can create an interactive dashboard with ShinyApp that integrates the leaflet with the graphs. 
...
Currently, summary statistics (standard deviation/min/max) of the same root trait are counted as different root traits. A possible step we could take is to clean the data set to consider summary statistics of the same variable as a single root trait instead of treating them separately. This can result in changes in the bar graph of the most commonly recorded root traits, in addition to the time series graph showing the number of root traits recorded over the years. We could compare the graphs that we have created that treat summary statistics of a root trait as their own root trait to the graphs that group them as a single root trait type.
...
First-Person Science: Colleen Iversen on Belowground Ecology. (2020, June 30). Department of Energy. https://www.energy.gov/science/articles/first-person-science-colleen-iversen-belowground-ecology
